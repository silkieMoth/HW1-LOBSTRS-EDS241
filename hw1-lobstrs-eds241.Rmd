...
The poisson model more accurately explains the relationship between the treatment and and dependent variable, as rather there being a defined change between MPA and non-MPA zones, there is a chance that a count observation is in either. The p-value does decrease from the linear model to the poisson model, even though it was low before. This is likely due to the produced line just being a better fit along the data.
...
Because a poisson model better represents the relationships in the data, the line generated is a better fit and therefore the residuals are approximately homoscedastic, as suggested by the homogeneity of variance graph. But the uniformity of residuals graph suggests that the line still isn't a very good fit, especially at the tails. This is further suggested by the posterior predictive check. The dispersion and zero-inflation graph also suggests a high degree of either. There also may be a few outliers; cook's distance identified 5 of them.

**f.** Conduct tests for over-dispersion & zero-inflation. Explain results.

There is a high degree of over-dispersion, potentially due to omitted variable bias. For the zero-inflation test, because the response variable isn't discrete or binary the zero-inflation test does not apply.
...
The data violates a couple assumptions about poisson distributions, being mean != variance (overdispersion), and no zero counts. A negative binomial model is an alternative in which these violations are accounted for.

**i.** Interpret the treatment estimate result in your own words. Compare with results from the previous model.

When selecting a count value at random, there is a 28% difference in the chance of that count value being from either the treatment sites or the non-treatment sites, with it being more likely to be from the treatment sites if it's lower, and vice versa if it's higher. The poisson model actually had the same coefficient, but because many assumptions about poisson were violated, the line was not a good fit.
...
While the interpretation of the coefficients is different between the OLS and poisson/negative-binomial models, the general conclusion still holds true between all three, which is that the treatment is associated with lower lobster counts. This would make the treatment effect robust. The biggest difference between the models is the SE for OLS being so large, likely because it is not a well fitting line for the data.
...
Overall, the coefficients for treatment suggests that there is a positive correlation between MPAs and lobster counts when accounting for current year. The year20XX coefficients show the total change from the previous year. The treat:year20XX coefficients show what the difference of the average counts between the MPA and non-MPA sites in a given year.

**d.** Explain why the main effect for treatment is negative? *Does this result make sense?

Whether the the main effect coefficients are negative or positive doesn't matter that much, but it does make sense for several reasons. One, it represents the **difference** between treatment and non-treatment. Second, overall it seems that overall the treatment lobster counts are lower than the non-treatment.
...
plot_counts <- spiny_counts %>% 
    ...
    mutate(mean_count = mean(counts),
           year = as_factor(year)) %>%
    summarize(mean_count = first(mean_count))

# plot_counts %>% ggplot() ...

plot_counts %>%
    ggplot(aes(x = year, y = mean_count, group = mpa, color = mpa)) + 
    geom_point(size = 4) + 
...
```
    ...
    There does not appear to be any meaningful difference between the 2018 regressions and the 2024 regressions. They all experience the same problems (or don't) regarding homoskedacity, residual variance and overdispersion. They also arrive at the same conclusions. 
    The only differences between the two datasets are that:
        *   The 2024 data is more nonlinear
        *   The poisson model for 2024 has fewer outliers
